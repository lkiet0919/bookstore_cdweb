...
- Đã cấu hình IP tĩnh cho máy ảo
  - Trong Ubuntu thì cấu hình file mạng tại:
    ```shell
    lkiet0919@dev-server:~$ sudo nano /etc/netplan/00-installer-config.yaml
    ```
  ...
  - Cấu hình xong thì apply với lệnh:
    ```shell
    lkiet0919@dev-server:~$ sudo netplan apply
    ```
...
```shell
PS C:\Users\kiet> scp todolist.zip lkiet0919@192.168.1.69:/home/lkiet0919
```
...
```shell
lkiet0919@dev-server:~$ sudo apt update && sudo apt install unzip # cài đặt unzip
lkiet0919@dev-server:~$ unzip todolist.zip # giải nén file todolist.zip
```
...
```shell
lkiet0919@dev-server:~$ sudo adduser todolist
lkiet0919@dev-server:~$ sudo mkdir /projects
lkiet0919@dev-server:~$ sudo mv todolist /projects/todolist
lkiet0919@dev-server:~$ sudo chown -R todolist:todolist /projects/todolist
lkiet0919@dev-server:~$ sudo chmod -R 750 /projects/todolist/
lkiet0919@dev-server:/projects/todolist$ cd  /projects/todolist
```

Để chạy được dự án front-end thì cần cài đặt nodejs và nginx

```shell
lkiet0919@dev-server:/projects/todolist$ sudo apt install -y nodejs npm nginx
...
```shell
lkiet0919@dev-server:/projects/todolist$ sudo nano /etc/nginx/sites-available/default
```
...
```shell
lkiet0919@dev-server:/projects/todolist$ sudo nano /etc/nginx/conf.d/todolist.conf
```
...
```shell
lkiet0919@dev-server:/projects/todolist$ sudo usermod -aG todolist www-data
```

Cuối cùng, chúng ta sẽ kiểm tra syntax xem có lỗi sai không và chạy lại dịch vụ nginx:

```shell
lkiet0919@dev-server:/projects/todolist$ sudo nginx -t
nginx: the configuration file /etc/nginx/nginx.conf syntax is ok
nginx: configuration file /etc/nginx/nginx.conf test is successful
lkiet0919@dev-server:/projects/todolist$ sudo systemctl restart nginx
```
...
1. Copy dự án sang server linux:
  ...
  ```shell
  PS C:\Users\kiet\Desktop> scp shoeshop-ecommerce.zip lkiet0919@192.168.100.69:~/
  ...
  This host key is known by the following other names/addresses:
    C:\Users\kiet/.ssh/known_hosts:8: 192.168.101.69
    C:\Users\kiet/.ssh/known_hosts:10: 192.168.1.69
  Are you sure you want to continue connecting (yes/no/[fingerprint])?
  Warning: Permanently added '192.168.100.69' (ED25519) to the list of known hosts.
  lkiet0919@192.168.100.69's password:
  shoeshop-ecommerce.zip                                                                100%   25MB  54.7MB/s   00:00
  PS C:\Users\kiet\Desktop> ssh lkiet0919@192.168.100.69
  ```
...
2. Update lại apt và cài đặt unzip để giải nén
  ...
  root@dev-server:~# cd /home/lkiet0919
  ```

3. Giải nén file zip, tạo thư mục cho project và user tương ứng

  ```shell
  root@dev-server:/home/lkiet0919# unzip shoeshop-ecommerce.zip
  root@dev-server:/home/lkiet0919# mkdir /projects && mv ./shoeshop /projects/shoeshop
  root@dev-server:/home/lkiet0919# adduser shoeshop
  root@dev-server:/home/lkiet0919# chmod -R 750 /projects/shoeshop && chown -R shoeshop:shoeshop /projects/shoeshop
  ```
...
1. **Cài đặt jdk 8**
  ...
  ```shell
  root@dev-server:/home/lkiet0919# apt-get install -y openjdk-8-jdk
  ```
...
2. **Cài đặt maven, tại đây sẽ cài thủ công thay vì qua apt** (chỉ để giới thiệu là còn cách cài khác trong trường hợp gói không có sẵn trong apt)
  ...
  ```shell
  root@dev-server:/home/lkiet0919# wget https://dlcdn.apache.org/maven/maven-3/3.9.9/binaries/apache-maven-3.9.9-bin.tar.gz
  root@dev-server:/home/lkiet0919# tar -xvf apache-maven-3.9.9-bin.tar.gz -C /opt/
  root@dev-server:/home/lkiet0919# ls /opt 
  apache-maven-3.9.9
  root@dev-server:/home/lkiet0919# echo 'export M2_HOME="/opt/apache-maven-3.9.9"' > /etc/profile.d/myscript.sh
  root@dev-server:/home/lkiet0919# echo 'export PATH="$PATH:$M2_HOME/bin"' >> /etc/profile.d/myscript.sh
  root@dev-server:/home/lkiet0919# source /etc/profile.d/myscript.sh
  ```
...
3. **Cài đặt mysql**
  ...
  ```shell
  root@dev-server:/home/lkiet0919# apt install -y mysql-server
  root@dev-server:/home/lkiet0919# vi /etc/mysql/mysql.conf.d/mysqld.cnf
  root@dev-server:/home/lkiet0919# systemctl restart mysql
  root@dev-server:/home/lkiet0919# netstat -tlunp
...
4. **Load dữ liệu cho mysql** 
  ...
  ```shell
  root@dev-server:/home/lkiet0919# mysql -u root
...
Sau khi chuẩn bị xong database, có thể đăng nhập bằng user vừa tạo rồi tiến hành load dữ liệu từ file sql.
  ...
  ```shell
  root@dev-server:/home/lkiet0919# su shoeshop && cd /projects/shoeshop
